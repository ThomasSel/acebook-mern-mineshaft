...
describe("/posts", () => {
  ...
  describe("DELETE, with a token", () => {
    it("removes all posts form the databases", async () => {
      let post1 = new Post({ message: "howdy!" });
      let post2 = new Post({ message: "hola!" });
      await post1.save();
      await post2.save();
  ...
  describe("DELETE, without a token", () => {
    it("Doesn't change the database", async () => {
      let post1 = new Post({ message: "howdy!" });
      let post2 = new Post({ message: "hola!" });
      await post1.save();
      await post2.save();
      let response = await request(app).del("/posts");
      Post.find(async (err, posts) => {
        if (err) throw err;
