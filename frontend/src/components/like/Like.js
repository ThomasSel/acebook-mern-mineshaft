...
function Like(props) {
  ...
  function parseJwt(token) {
    if (!token) {
      return { user_id: "fakeId" };
    }
    const base64Url = token.split(".")[1];
    if (!base64Url) return { user_id: "fakeId" };
    ...
    <div id="like-button" class="flex py-2 text-center">
      <Fragment>
        <button
          // href="#"
          onClick={handleClick}
          class="group mt-1 flex w-auto items-center rounded-full px-3 py-2 text-base font-medium leading-6 text-gray-500 hover:bg-blue-800 hover:text-blue-300"
        >
          <svg
